...
- **2026-09-13 — The public/gated line MOVES: the passport now shows evidence, issuers and component traceability.** This supersedes the 2026-09-06 disclosure-model entry, which put "evidence document references, supplier/sourced-from identities, component weights and full BOM composition" firmly on the gated side. Sprint 10 deliberately reverses that for four classes of fact, on the owner's instruction:
  ...
  · **Component traceability** — origin, maker, mass, recycled share;
  · **Key value** — a measured figure against the rule's limit, where both exist.
  *Rationale:* "Qualified" on its own is the weakest useful thing a passport can say. A reader who cannot tell an accredited laboratory's test report from the supplier's own declaration will assume the stronger one, and the page will have misled them without stating anything false. Disclosing who stands behind a claim is what makes the claim checkable, which is the entire purpose of a passport reached by QR at a loading dock.
  **What stays gated, and the rule that decides it:** document *bodies*, contact details, the assessor's risk-annotation rationale, delta-action wording, and any measured value beyond the single key-value line. The line is now **identity and outcome are public; narrative, contact routes and raw documents are not.** A reader may learn *who* and *what*, never *how to reach them* or *what else was said*.
  This is a disclosure decision, not an evaluation one — no engine, corpus or extraction code changed in this sprint.
- **2026-09-13 — ICE is REMOVED from consideration as an emission-factor source.** Circular Ecology's own page states that non-educational use of ICE Database data **"will no longer be permitted after 30th September 2026"**; the Educational database and its derivatives permit **no commercial use**, and Advanced-version users may not distribute derived works to unregistered parties without written permission — which is exactly what a public passport carrying an ICE value would be. This screening tool is a commercial product, so ICE is out: not "attribution required", but unusable. It was the **only** public source in Climatiq `^36` for wood, plywood and MDF/particle-board, so its removal is what forces the BEIS generic-wood proxy below. Revisit only if an IC+ (or equivalent written) licence is bought. Terms read 2026-09-13 and recorded verbatim in [docs/reference/factor-dataset-licences.md](docs/reference/factor-dataset-licences.md).
- **2026-09-13 — Selected factor set is BEIS material-use (OGL v3.0), primary-material-production rows, value display permitted.** OGL v3.0 permits copying, publishing, distributing and adapting with Crown copyright attribution, so these are the only shortlisted factors whose **value** may appear on the public passport. ICM (CC-BY, also permissive) was **considered and rejected** on data fit: every ICM row is Australian, 2019, and `cradle_to_shelf` — a wider system boundary than the "cradle-to-gate" the report claims, which would make the label untrue rather than merely imprecise.
- **2026-09-13 — Wood uses the BEIS GENERIC wood row as a LABELLED PROXY.** With ICE removed, no public dataset in `^36` distinguishes sawn softwood from plywood: BEIS publishes one `timber_forestry-type_wood_primary_material_production` (0.269504 kgCO2e/kg, GB, 2026) covering wood generally. It is selected for both `wood_solid` and `wood_processed` and **labelled a proxy in the factor's notes**, because a proxy presented as a match is the kind of silent error this project exists to prevent. (Sanity check: it lands within 2.5% of the ICE softwood figure it replaces.) MUF presswood has no public row at all and waits on a Fitsol primary factor.
- **2026-09-13 — Factor granularity is per BOM MATERIAL, not per material variant — post-Tuesday.** The shortlists were drawn per variant (corrugated board vs kraft liner; LDPE film vs PET strap; plywood vs MUF presswood), but `emission_factors` is keyed `(material, process)` and the BOM vocabulary has five materials. So the carton, the inner fitting and the kraft label are all `corrugated` and share ONE factor, and the LDPE bag and the PET strap are both `plastic`. Selecting a second factor for the same BOM material does not add a choice — it appends a version that **supersedes** the first for every component of that material. For the demo this is resolved by choosing the dominant variant per material (board for corrugated, PET for plastic — the heavier of the two in both packs that contain them). The real fix is a component-level factor override or a finer material vocabulary, and it is deferred: doing it properly touches the BOM schema, the intake form and the engine's resolution order.
- **2026-09-13 — `metal` uses the BEIS GENERIC metals row as a labelled proxy; BEIS has no steel primary-production row.** Source-filtered search of BEIS `^36` public returns steel only as `metal_products-type_steel_cans_closed_loop_source` — a **closed-loop** (recycled-content) figure, not comparable with the primary-material-production rows used for every other material, and mixing the two would make the pack total incoherent. The only metals primary row is generic: `metals-type_primary_material_production`, 3821.94858 kg/tonne → **3.82195 kgCO2e/kg**, GB 2026. Selected for `metal` and labelled a proxy. Also rejected: `metals-type_basic_iron_and_steel`, which is **spend-based (kg/gbp)** and is refused by the unit guard because it cannot become a per-kg factor; and `metals-type_scrap_metal_primary_material_production` (3.47091), scrap being a different input stream from new fasteners. With this row selected, every component in all three demo packs resolves and no card reads "partial".
- **2026-09-13 — Recycled-content blend rule: POST-TUESDAY commit, not before.** BEIS publishes `primary_material_production` and `closed_loop_source` separately, and the gap is large (PET 3.86 vs 2.21 kgCO2e/kg). The right model is `factor = primary × (1 − r) + closed_loop × r`, where `r` is the component's recycled-content fraction. That needs a `recycled_content_fraction` on the component, a second factor slot per material, and a report line showing the blend — schema plus engine plus display. Until it lands, every selection is the **primary** row, which is the conservative end: a screening that overstates a footprint is a safer error than one that understates it. Deliberately deferred past the 15 Sep demo.
- **2026-09-13 — Factor selection must be run TWICE: locally and against production.** `factors:select` is a database write, and local and production hold separate stores — merging a branch ships the code, never the factors. The owner runs the same commands with `DATABASE_URL` pointed at each, then `npm run seed:demo-suite` against each. The re-seed is **required, not cosmetic**: `assessments.factors_pinned_at` is first-evaluation-wins, so any demo pack whose report has already been opened is pinned to the empty factor set and will keep rendering "No factor selected" until it is recreated.
